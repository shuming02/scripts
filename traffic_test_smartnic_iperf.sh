...
#example on the source host
# ssh-copy-id -i ~/.ssh/id_rsa.pub shuming02@source
# ssh-copy-id -i ~/.ssh/id_rsa.pub shuming02@dst
...
function create_one()
{
	...
	--data '{"ignoreResourceCheck":"test", "skipLicenseCheck":true, "appkey": "com.sankuai.inf.hulk.demo", "idc": "'$idc_name'", "env": "dev","num": 1,"cpu": 4,"mem": 4096,"hd": 200,"clusterType": "hulk_kernal_qa"  ,"additional":{"requireAffinities":[{"key":"kubernetes.io/hostname","inValues":["'$1.mt'"],"operator":"IN"}] }}'`
...
do
        ...
        ssh -t shuming02@$host1  sudo -iu sankuai  sudo -s  docker exec -itd $id1 nohup iperf -s
        ssh -t shuming02@$host2 sudo -iu sankuai sudo -s  docker exec -itd $id2 iperf -c $container1 -t $interval
done
...
do
        ...
        ssh -t shuming02@$host1 sudo -iu sankuai sudo -s docker exec -it $id1 cat /nohup.out > tmp_outputs.txt
