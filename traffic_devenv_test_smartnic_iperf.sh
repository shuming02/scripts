#!/bin/bash
#please use ssh-keygen to generate the public keys to the remote servers
...
function create_one()
{
	...
	--data '{"ignoreResourceCheck":"test", "skipLicenseCheck":true, "appkey": "com.sankuai.inf.hulk.demo", "idc": "zf","env": "dev","num": 1,"cpu": 4,"mem": 4096,"hd": 200,"clusterType": "default"  ,"additional":{"requireAffinities":[{"key":"kubernetes.io/hostname","inValues":["'$1.mt'"],"operator":"IN"}] }}'`
...
do
        echo id=$id1
        ssh -t shuming02@$host1 sudo -iu sankuai sudo -s docker exec -it $id1 cat /nohup.out >> all_outputs.txt
done
