...
actual_num=0
bandwith=1000
total_packets=`expr $interval \* $bandwith`
...
function create_one()
{
	...
	--data '{"ignoreResourceCheck":"test", "skipLicenseCheck":true, "appkey": "com.sankuai.inf.hulk.demo", "idc": "zf","env": "test","num": 1,"cpu": 4,"mem": 4096,"hd": 200,"clusterType": "default"  ,"additional":{ "requireAffinities":[{"key":"kubernetes.io/hostname","inValues":["'$1.mt'"],"operator":"IN"}] }}'`
...
function destroy_containers()
{
        ...
        do
 	       ...
 	       --header 'Content-Type: application/json' \
 	       --header 'auth-token: '$token'' \
 	       --data '{"appkey": "com.sankuai.inf.hulk.demo","setNames": ["'$container_name'"]}'`
...
do
	counter=$[ $counter - 1 ]
        ...
        ssh -t shuming02@$host1 "sudo -iu sankuai sudo -s  docker exec -it $cid_found /usr/bin/yum -y install iperf3"
	...
	#install iperf to the new created containers
        ...
        ssh -t shuming02@$host2 "sudo -iu sankuai sudo -s  docker exec -it $cid_found /usr/bin/yum -y install iperf3"
        sleep 5
...
for container1 in $container_hosts1
do
        ids2_next=${ids2_next#*\ }
        ids1_next=${ids1_next#*\ }
        id1=${ids1_next%%\ *}
        ...
        ssh -t shuming02@$host1  sudo -iu sankuai  sudo -s  docker exec -itd $id1 nohup iperf3 -s
        #ssh -t shuming02@$host2 sudo -iu sankuai sudo -s  docker exec -itd $id2 iperf3 -l 64 -u -b 1000M -n 10G -c $container1 -t $interval
        ssh -t shuming02@$host2 sudo -iu sankuai sudo -s  docker exec -itd $id2 nohup iperf3 -l 64 -u -b "$bandwith"M -n "$total_packets"M -c $container1
done
...
do
        echo id=$id1
        ssh -t shuming02@$host1 sudo -iu sankuai sudo -s docker exec -it $id1 cat /nohup.out >> server_outputs.txt
done
for id2 in $container_ids2
do
        echo id=$id2
        ssh -t shuming02@$host2 sudo -iu sankuai sudo -s docker exec -it $id2 cat /nohup.out >> client_outputs.txt
done
