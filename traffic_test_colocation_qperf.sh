...
function create_one()
{
	...
	--data '{"ignoreResourceCheck":"test", "skipLicenseCheck":true, "appkey": "com.sankuai.inf.hulk.demo", "idc": "gh","env": "test","num": 1,"cpu": 4,"mem": 4096,"hd": 200,"clusterType": "default"  ,"additional":{     "dedicatedPool":["kernel_test_colocation_alpha"],    "requireAffinities":[{"key":"kubernetes.io/hostname","inValues":["'$1.mt'"],"operator":"IN"}] }}'`
...
do
        ...
        ssh -t shuming02@$host1  sudo -iu sankuai  sudo -s  docker exec -itd $id1 nohup iperf -s -u
        ssh -t shuming02@$host2 sudo -iu sankuai sudo -s  docker exec -itd $id2 iperf -l 64 -P 10 -u -b 1000M -c $container1 -t $interval
done
